package com.sampleapp.event.listener;
...
import com.sampleapp.exception.MailSendException;
import com.sampleapp.model.PasswordResetToken;
import com.sampleapp.model.User;
import com.sampleapp.service.MailService;
...
public class OnGenerateResetLinkEventListener implements ApplicationListener<OnGenerateResetLinkEvent> {
    ...
    private void sendResetLink(OnGenerateResetLinkEvent event) {
        ...
        String emailConfirmationUrl = event.getRedirectUrl().queryParam("token", passwordResetToken.getToken())
                .toUriString();
        try {
            mailService.sendResetLink(emailConfirmationUrl, recipientAddress,passwordResetToken.getToken());
